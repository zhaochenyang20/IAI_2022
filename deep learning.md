...
<img src="./pic/deep_learning/CNN1.jpg" style="zoom:25%;" />

### CNN 的特点

1. 局部链接：每个神经元仅与输入神经元的一块区域连接，这块局部区域称作感受（receptive field）。
2. 权值共享：同一层的权值相同，移动视野不移动权重。
3. 池化：最大池化 or 平均池化
4. 从局部到全局：层数越高的神经元获得的感知范围越大。
5. CNN 同样使用 BP 来优化，推导相对复杂
6. 参数少，只与卷积核的大小和数量有关
7. 具有特征抽取能力
8. 特征的平移不变性（一定程度上）

### [channel](https://blog.csdn.net/sscc_learning/article/details/79814146)
...
一般来说，当高的步幅 $s_h$，宽的步幅 $s_w$，高两侧的填充 $p_h$，宽两侧填充 $p_w$，卷积核形状为 $k_h \times k_w$，则输出大小为：
$$
