...
![RNN](./pic/exam/RNN.jpg)

- 设计如图所示的 RNN 神经网络，将输入张量逐列与参数矩阵 U 相乘输入记忆神经元，然后经过 tanh 激活，再与 W 矩阵相乘后记忆下来，与新输入的列与 U 的乘积相加。先输入的列得到了记忆，并且利用了后输入的列，构造了循环神经网络。最后循环神经元输出的结果与参数矩阵 V 相乘，经过 softmax 输出概率。计算损失函数而后回传梯度下降。
